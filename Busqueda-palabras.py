...
indice = {}
# Inicio del bucle para procesar cada documento.
for i, documento in enumerate(documentos, start=1):
    palabras = documento.lower().split()
    # Inicio del bucle para procesar cada palabra en el documento.
    for palabra in palabras:
        if palabra not in indice:
            indice[palabra] = []
        indice[palabra].append(i)
        # Si la palabra no existe en el índice, se crea una entrada con una lista vacía.
        # Luego, se agrega el número de fila actual a la lista correspondiente a la palabra.

# Imprimir el índice generado.
for palabra, documentos_con_palabra in indice.items():
    print(f"Palabra: {palabra}")
    print(f" Se encuentra en la fila: {documentos_con_palabra}")
    print("-" * 30)
    # Imprimir cada palabra junto con los documentos en los que aparece.

import time

inicio = time.time()
# Aquí se registra el tiempo de inicio de la ejecución.
# A continuación, se realizaría el procesamiento principal.
fin = time.time()
# Aquí se registra el tiempo de finalización de la ejecución.
